...
class MutationAnnotator:
    ...
    def annotate_with_methylation(self, mutations_df: pd.DataFrame, 
        ...
        if len(meth_df.columns) == 4:  # BED format without header
            meth_df.columns = ['chr', 'start', 'end', 'methylation_level']
        elif 'methylation_class' not in meth_df.columns:
            # Classify on the fly if not already done
            conditions = [
        ...
        if len(intersected) > 0:
            ...
            for _, row in intersect_df.iterrows():
                ...
                if mutation_idx < len(mutations_df):
                    mutations_df.loc[mutation_idx, 'methylation_level'] = row['score']  # Assuming score contains methylation_level
                    mutations_df.loc[mutation_idx, 'methylation_class'] = row['strand']  # Using strand field for class
...
def main():
    parser = argparse.ArgumentParser(description='Annotate mutations with methylation states and functional consequences')
